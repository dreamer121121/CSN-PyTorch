...
best_prec1 = 0
log_stream = open(os.path.join(args.checkpoint_dir, 'log', '%s.txt' % store_name), 'a')
def main():
    ...
    #****************************Create Model***************************
    model = getattr(CSN,args.arch)(num_class,target_transforms = target_transforms,mode=args.mode)
    ...
    if torch.cuda.is_available():
        ...
        if len(devices) > 1:
            model = torch.nn.DataParallel(model,device_ids=devices) # 使用单机多卡进行训练
    else:
        device = 'cpu'
...
def save_checkpoint(state, is_best, filename):
    torch.save(state,
               '%s/%s_checkpoint_%s.pth.tar' % (os.path.join(args.checkpoint_dir, 'checkpoint'), store_name, filename))
    if is_best:
        shutil.copyfile(
            '%s/%s_checkpoint_%s.pth.tar' % (os.path.join(args.checkpoint_dir, 'checkpoint'), store_name, filename),
...
def accuracy(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        pred = pred.t()  # pred ==>5X16
        correct = pred.eq(target.view(1, -1).expand_as(pred))

        res = []
        for k in topk:
            correct_k = correct[:k].view(-1).float().sum(0, keepdim=True)
            res.append(correct_k.mul_(100.0 / batch_size))  # 已经转换为百分数了
        return res
